export const focusAreaDetails = {
  "maritime-law": {
    mainImage: "/maritime-law.png",
    title: "Maritime Law",
    intro:
      "Maritime law covers a wide range of activities, including the carriage of goods and passengers by sea, marine insurance, ship registration, and maritime safety. It ensures the smooth operation of maritime activities, protects the rights and interests of parties involved, and regulates conduct on the high seas and inland waterways.",

    sections: [
      {
        heading: "1.1 What is it?",
        content: `
          Maritime law covers a wide range of activities, including the carriage of goods and passengers by sea, marine insurance, ship registration, and maritime safety. 
          It ensures the smooth operation of maritime activities, protects the rights and interests of parties involved, and regulates conduct on the high seas and inland waterways.
          
          **Key Distinctions:**
          * **Maritime Law vs. Law of the Seas:** The law of the seas is a set of international rules applying to nations on territorial seas. Maritime law is a **private law** that applies to individuals and institutions.
          * **Admiralty Law:** Maritime law is also known as admiralty law, governing nautical issues arising on navigable waters.

          **Key Objectives:**
          1.  Regulation of Shipping Activities: Ensuring safe and efficient shipping operations.
          2.  Protection of Maritime Interests: Safeguarding the rights and interests of shipowners, cargo owners, and other stakeholders.
          3.  Maritime Safety: Promoting the safety of ships, crews, and passengers.
          4.  Environmental Protection: Preventing and mitigating marine pollution.
        `
      },
      {
        heading: "1.2 Areas covered?",
        content: `Maritime law encompasses a wide range of legal issues that arise from shipping, navigation, and the use of the sea, drawing from contract, tort, insurance, and environmental law.`,
        subSections: [
          {
            title: "Maritime Contracts",
            details: `Regulating commercial relationships via charterparty agreements, bills of lading, marine insurance contracts, and towage agreements. Governed by the Merchant Shipping Act 2007 and international conventions.`
          },
          {
            title: "Admiralty Jurisdiction and Procedure",
            details: `Defining the scope of maritime cases courts can hear, including in rem actions (against a vessel), in personam actions, and the arrest and judicial sale of ships. The Federal High Court holds exclusive jurisdiction.`
          },
          {
            title: "Alternative Dispute Resolution (ADR)",
            details: `Utilizing arbitration, mediation, and negotiation, often favored for being faster, confidential, and handled by subject-matter experts. Common arbitration centers include LMAA and LACIAC.`
          },
          {
            title: "Maritime Labour and Employment",
            details: `Covering the rights and welfare of seafarers, including contracts, wages, repatriation, and safety standards, governed by the Merchant Shipping Act and the Maritime Labour Convention (MLC).`
          },
          {
            title: "Maritime Torts and Liability",
            details: `Addressing civil wrongs like collisions, negligence, cargo damage, and pollution. Liability may be limited by international conventions.`
          },
          {
            title: "Marine Environmental Protection",
            details: `Dealing with the control and prevention of marine pollution, including oil spills and waste discharge, implemented through NIMASA and conventions like MARPOL.`
          }
        ]
      },
      {
        heading: "1.3 Legal Framework - Key Laws",
        isList: true,
        listItems: [
          "Constitution of the Federal Republic of Nigeria 1999 (Exclusive Legislative List, Federal High Court jurisdiction).",
          "Merchant Shipping Act 2007 (Principal law on shipping regulation, modernizing standards on registration, safety, and liability).",
          "Nigerian Maritime Administration and Safety Agency (NIMASA) Act 2007 (Establishes NIMASA to enforce international conventions and regulate safety/security).",
          "Admiralty Jurisdiction Act 1991 and Rules (Governs maritime litigation, ship arrest, and enforcement).",
          "Coastal and Inland Shipping (‘Cabotage’) Act of 2003 (Restricts foreign vessels in domestic coastal trade and promotes indigenous tonnage)."
        ]
      },
      {
        heading: "1.4 Who are our clients?",
        isList: true,
        listItems: [
          "Shipping companies and shipowners",
          "Charterers and time/voyage operators",
          "Freight forwarders, clearing & forwarding agents",
          "Terminal & stevedoring operators",
          "Oil & gas service companies and offshore support operators",
          "Insurers and P&I clubs",
          "Financial institutions financing maritime assets"
        ]
      },
      {
        heading: "1.6 How can we help?",
        content: `We provide strategic and practical legal solutions tailored to prevent disputes, ensure compliance, and protect commercial interests.`,
        subSections: [
          {
            title: "Alternative Dispute Resolution (ADR)",
            details:
              "Resolving disputes through arbitration, mediation, and negotiation (demurrage, cargo damage, charter party disputes)."
          },
          {
            title: "Contract Drafting and Transactional Support",
            details:
              "Drafting, reviewing, and negotiating charter parties, bills of lading, and marine insurance policies."
          },
          {
            title: "Regulatory and Compliance Advisory",
            details:
              "Guiding clients through compliance with NIMASA, NPA, and customs documentation (vessel registration, cabotage)."
          },
          {
            title: "Ship Arrest and Release Advisory",
            details:
              "Providing advisory support and coordinating with litigation partners during ship arrest matters."
          },
          {
            title: "Marine Insurance and Risk Advisory",
            details:
              "Advising on marine insurance coverage, liability issues, and risk allocation."
          },
          {
            title: "Representation Before Regulatory Authorities",
            details:
              "Interfacing with NIMASA, NPA, and other agencies on behalf of clients."
          }
        ]
      }
    ]
  },

  "entertainment-law": {
    mainImage: "/entertainment-law.png",
    title: "Entertainment Law",
    intro:
      "Entertainment Law governs the creation, production, distribution, and commercial exploitation of creative works in the entertainment industry, ensuring creativity and commerce coexist under clear legal frameworks.",

    sections: [
      {
        heading: "2.1 What is it?",
        content: `
          Entertainment Law refers to the body of legal principles and regulations that govern the creation, production, distribution, and commercial exploitation of creative works in the entertainment industry. 
          It provides legal protection for the rights and interests of individuals and businesses involved in music, film, television, theatre, sports, fashion, and digital media.
        `
      },
      {
        heading: "2.2 Areas covered?",
        content: `Entertainment law encompasses a wide range of legal disciplines, adapting general legal principles to suit the unique dynamics of the creative sector.`,
        subSections: [
          {
            title: "Contract Law",
            details:
              "Defining rights and obligations of artists, producers, managers, and studios via recording agreements, production deals, and endorsement contracts."
          },
          {
            title: "Intellectual Property (IP) Law",
            details:
              "The backbone of the industry, protecting works via copyrights, trademarks, and related rights (ownership, licensing, enforcement)."
          },
          {
            title: "Alternative Dispute Resolution (ADR)",
            details:
              "Using mediation and arbitration for sensitive disputes like contractual breaches and royalty issues, prioritizing speed and confidentiality."
          },
          {
            title: "Licensing and Royalties",
            details:
              "Governing the negotiation, drafting, and enforcement of agreements for commercial exploitation (broadcasting, streaming, synchronization)."
          },
          {
            title: "Labour and Talent Management",
            details:
              "Regulating relationships between talent and their agents/managers, covering employment contracts and professional conduct."
          },
          {
            title: "Defamation and Privacy",
            details:
              "Addressing reputation and privacy issues, including image rights, invasion of privacy, and misuse of likeness."
          },
          {
            title: "Technology and Digital Media",
            details:
              "Covering online distribution, influencer marketing, data protection, and IP in the digital space."
          }
        ]
      },
      {
        heading: "2.3 Legal framework - Key Laws",
        isList: true,
        listItems: [
          "Copyright Act 2022 (Governs ownership, reproduction, and use of creative works).",
          "Trade Marks Act (Protects stage names, logos, and brand identifiers).",
          "Companies and Allied Matters Act (CAMA) 2020 (Regulates company incorporation and governance).",
          "Nigerian Broadcasting Commission (NBC) Code (Regulates broadcast content and licensing).",
          "Nigerian Copyright Commission (NCC) (Oversees copyright registration and enforcement).",
          "Collecting Societies (e.g., COSON, MCSN) (Handle royalty collection and distribution)."
        ]
      },
      {
        heading: "2.4 Who are our clients?",
        isList: true,
        listItems: [
          "Record Labels and Music Production Companies",
          "Artistes, Musicians, Songwriters, and Producers",
          "Event Management Companies and Promoters",
          "Talent Management Firms",
          "Media and Production Houses",
          "Influencers and Content Creators",
          "Digital Streaming Platforms"
        ]
      },
      {
        heading: "2.6 How can we help?",
        content: `We help clients protect their rights, maximize earnings, and minimize risks by bridging the gap between creativity and compliance.`,
        subSections: [
          {
            title: "Contract Negotiation",
            details:
              "Ensuring all deals are transparent and beneficial to our clients, providing strategic legal advice for partnerships."
          },
          {
            title: "Intellectual Property Protection",
            details:
              "Protecting copyrights and trademarks from unauthorized use or exploitation."
          },
          {
            title: "Career Stage Support",
            details:
              "Supporting clients through every stage of their career, from content creation to commercial execution."
          }
        ]
      }
    ]
  },
  "property-law": {
    mainImage: "/property-law.png",
    title: "Property Law",
    intro:
      "Property Law governs the ownership, use, transfer, and management of land and other immovable property. It ensures all transactions relating to real estate are carried out lawfully, rights are properly transferred, and interests are adequately protected.",

    sections: [
      {
        heading: "3.1 What is it?",
        content: `
          Property Law is the branch of law that governs the ownership, use, transfer, and management of land and other immovable property. It defines the legal relationships between individuals and entities regarding real estate, whether as owners, tenants, lessors, lessees, or occupiers.

          In simple terms, Property Law ensures that all transactions relating to land and buildings are carried out lawfully, rights are properly transferred, and interests are adequately protected. It provides the legal framework for buying, selling, leasing, developing, or mortgaging property in Nigeria.
        `
      },
      {
        heading: "3.2 Areas covered?",
        content: `Property law governs the ownership, transfer, and use of land and real estate, as well as the rights and interests that arise in connection with them.`,
        subSections: [
          {
            title: "Conveyancing and Assignment",
            details:
              "Legal transfer of ownership or interest, including drafting and reviewing Deeds of Assignment and Deeds of Conveyance."
          },
          {
            title: "Leases and Tenancy Agreements",
            details:
              "Regulating landlord–tenant relationships, outlining rights, duties, and remedies for both parties in residential and commercial arrangements."
          },
          {
            title: "Land Acquisition and Title Perfection",
            details:
              "The legal process of acquiring land and perfecting title documents through registration and obtaining the Governor’s Consent."
          },
          {
            title: "Mortgages and Charges",
            details:
              "Governing the creation, registration, enforcement, and discharge of mortgages and charges over property as security for loans."
          },
          {
            title: "Land Use and Planning Regulations",
            details:
              "Covering building approvals, environmental regulations, and compliance with zoning laws under relevant statutes."
          },
          {
            title: "Property Development and Joint Ventures",
            details:
              "Governing collaborations between landowners, developers, and investors, ensuring proper documentation and risk allocation."
          },
          {
            title: "Succession and Property Inheritance",
            details:
              "Determining how property is transferred upon the death of an owner through wills, intestacy, or trust arrangements."
          }
        ]
      },
      {
        heading: "3.3 Legal Framework - Key Laws",
        isList: true,
        listItems: [
          "Land Use Act (Cap L5 LFN 2004): Vests ownership of all land in the state Governor.",
          "Property and Conveyancing Law (PCL): Applicable in some states, governing conveyancing, leases, and mortgages.",
          "Land Registration Laws of Various States: Regulate documentation, registration, and perfection of land titles.",
          "Mortgage and Property Law of Lagos State Cap M5 2010: Modernizes property and mortgage transactions, simplifying creation and enforcement of mortgages.",
          "Tenancy Law of Lagos State (Law No. 14 of 2011): Governs landlord-tenant relationships, eviction processes, and rent control (with exceptions).",
          "Wills Act 1837 and Wills Laws of Various States: Regulate testamentary disposition and inheritance."
        ]
      },
      {
        heading: "3.4 Who are our clients?",
        isList: true,
        listItems: [
          "Landlords and Tenants, Lessors and Lessees",
          "Property Developers and Real Estate Firms",
          "Surveyors, Valuers, Estate Agents, and Property Managers",
          "Corporate Organizations and Financial Institutions",
          "Individuals acquiring or disposing of property assets"
        ]
      },
      {
        heading: "3.6 How can we help?",
        content: `We assist clients in navigating the complexities of land transactions by providing reliable, efficient, and transparent legal solutions.`,
        subSections: [
          {
            title: "Proprietary Interest Protection",
            details:
              "Protecting clients’ interests through proper documentation and registration."
          },
          {
            title: "Dispute Prevention",
            details:
              "Preventing disputes through clear, well-drafted agreements."
          },
          {
            title: "Regulatory Compliance",
            details:
              "Ensuring compliance with statutory requirements and government regulations."
          },
          {
            title: "Strategic Advisory",
            details:
              "Providing strategic advice on real estate investment, leasing, and development."
          },
          {
            title: "Dispute Representation",
            details:
              "Representing clients in negotiation and mediation, and collaborating with trusted external counsel for litigation."
          }
        ]
      }
    ]
  },

  // --- 4.0 FAMILY LAW ---
  "family-law": {
    mainImage: "/family-law.png",
    title: "Family Law",
    intro:
      "Family law governs legal relationships within the family unit, addressing rights, duties, and obligations that arise between spouses, parents, and children. Its goal is to promote fairness, protect vulnerable members, and preserve the welfare of children and dependants.",

    sections: [
      {
        heading: "4.1 What is it?",
        content: `
          Family law governs legal relationships within the family unit, addressing rights, duties, and obligations that arise between spouses, parents, and children. 
          It also regulates marriage, divorce, maintenance, custody, inheritance, and protection from domestic abuse or violence. 
          The goal of family law is to promote fairness, protect vulnerable members of the family, and preserve the welfare of children and dependants.
        `
      },
      {
        heading: "4.2 Areas covered?",
        content: `Family law spans a wide range of personal and domestic issues that affect individuals and families.`,
        subSections: [
          {
            title: "Marriage and Matrimonial Causes",
            details:
              "Covering the formation, validity, and dissolution of marriages under statutory, customary, and Islamic law, including annulments and separations."
          },
          {
            title: "Divorce and Judicial Separation",
            details:
              "Legal processes for dissolving marriages, determining grounds for divorce, financial settlements, alimony, and property division."
          },
          {
            title: "Child Custody and Maintenance",
            details:
              "Determining custody, guardianship, access rights, and maintenance obligations to ensure the welfare and best interests of the child."
          },
          {
            title: "Domestic Violence and Protection Orders",
            details:
              "Providing protection from abuse through restraining orders, police intervention, and enforcement of victims’ rights under laws like the VAPP Act."
          },
          {
            title: "Paternity, Legitimacy, and Child Rights",
            details:
              "Addressing paternity establishment, legitimacy, child support, and recognition of children born out of wedlock."
          },
          {
            title: "ADR in Family Matters",
            details:
              "Encouraging mediation, reconciliation, and arbitration to resolve sensitive family disputes amicably."
          }
        ]
      },
      {
        heading: "4.3 Legal Framework - Key Laws",
        isList: true,
        listItems: [
          "Matrimonial Causes Act, Cap. M7, LFN 2004: Governs marriage, divorce, maintenance, and custody for statutory marriages.",
          "Marriage Act, Cap. M6, LFN 2004: Provides for the formal celebration and registration of statutory marriages.",
          "Child’s Rights Act, Cap. C50, LFN 2004: Protects the rights and welfare of children, including guardianship and adoption.",
          "Violence Against Persons (Prohibition) Act, Cap. V5, LFN 2015 (VAPP Act): Prohibits all forms of domestic violence and harmful practices.",
          "Customary and Sharia Laws: Apply in cases of customary or Islamic marriages, inheritance, and divorce."
        ]
      },
      {
        heading: "4.4 Who are our clients?",
        isList: true,
        listItems: [
          "Individuals and couples seeking guidance on marriage or divorce.",
          "Parents and guardians involved in custody or maintenance disputes.",
          "Dependants seeking protection or enforcement of rights.",
          "Legal entities requiring guidance in family-related matters."
        ]
      },
      {
        heading: "4.6 How can we help?",
        content: `We help clients navigate sensitive family matters with professionalism, compassion, and discretion, focusing on sound legal advice and amicable settlement.`,
        subSections: [
          {
            title: "Legal Advice and Documentation",
            details:
              "Offering sound legal advice on marriage, divorce, and maintenance issues, and preparing/reviewing wills and custody agreements."
          },
          {
            title: "Amicable Settlement",
            details:
              "Facilitating the amicable settlement of family disputes through mediation and negotiation."
          },
          {
            title: "Child Welfare Protection",
            details:
              "Ensuring the protection of children’s rights and welfare in all proceedings."
          },
          {
            title: "Coordinated Representation",
            details:
              "Collaborating with external counsel when litigation is necessary, ensuring strong and coordinated representation."
          }
        ]
      }
    ]
  },

  // --- 5.0 IMMIGRATION LAW ---
  "immigration-law": {
    mainImage: "/immigration-law.png",
    title: "Immigration Law",
    intro:
      "Immigration law regulates the entry, residence, employment, and exit of persons into and out of a country. Its objective is to ensure lawful migration, national security, and compliance with visa and work permit regulations.",

    sections: [
      {
        heading: "5.1 What is it?",
        content: `
          Immigration law regulates the entry, residence, employment, and exit of persons into and out of a country. 
          It governs how foreign nationals are admitted, their legal status while in the country, and the rights, duties, and procedures applicable to both individuals and corporate entities that employ or host them. 
          The objective is to ensure lawful migration, national security, and compliance with visa and work permit regulations.
        `
      },
      {
        heading: "5.2 Areas covered?",
        content: `Immigration law encompasses a broad range of administrative and regulatory issues involving individuals, families, and organizations.`,
        subSections: [
          {
            title: "Visa and Entry Clearance",
            details:
              "Advising on and assisting with obtaining various visa categories (tourist, business, student, work, investor) and entry compliance."
          },
          {
            title: "Residency and Work Permits",
            details:
              "Handling applications for Temporary/Permanent Residence Permits, and Combined Expatriate Residence Permits and Aliens Cards (CERPAC)."
          },
          {
            title: "Expatriate Quota and Business Immigration",
            details:
              "Assisting corporate clients in securing and managing Expatriate Quota Approvals for legally employing foreign professionals."
          },
          {
            title: "Immigration Compliance and Advisory",
            details:
              "Providing ongoing advisory services on regulations, employer compliance, documentation audits, and risk management."
          },
          {
            title: "Deportation, Repatriation, and Appeals",
            details:
              "Advising and supporting clients in contesting immigration violations, deportation orders, or visa denials."
          }
        ]
      },
      {
        heading: "5.3 Legal Framework - Key Laws",
        isList: true,
        listItems: [
          "Immigration Act, No. 8 of 2015: The principal legislation regulating entry, stay, and departure of persons in Nigeria.",
          "Nigerian Immigration Service (NIS): The main administrative agency responsible for immigration control, issuance of passports, visas, and residence permits.",
          "Federal Ministry of Interior: Oversees immigration policy formulation and the granting of expatriate quotas and citizenship."
        ]
      },
      {
        heading: "5.4 Who are our clients?",
        isList: true,
        listItems: [
          "Individuals and families seeking to migrate or reside legally.",
          "Expatriates and skilled workers.",
          "Multinational companies and corporate organizations hosting foreign employees.",
          "Foreign investors and entrepreneurs."
        ]
      },
      {
        heading: "5.6 How can we help?",
        content: `We offer practical, solution-driven immigration advisory and representation services designed to simplify compliance and protect clients’ legal interests.`,
        subSections: [
          {
            title: "Permit Applications",
            details:
              "Providing expert advice on visa, work, and residence permit applications."
          },
          {
            title: "Expatriate Quota Management",
            details: "Assisting with expatriate quota approvals and renewals."
          },
          {
            title: "Compliance Audits",
            details:
              "Offering corporate immigration compliance audits and policy guidance."
          },
          {
            title: "Liaison and Representation",
            details:
              "Representing clients before immigration authorities and embassies."
          },
          {
            title: "Strategic Collaboration",
            details:
              "Collaborating with external counsel or government liaisons where complex administrative review is required."
          }
        ]
      }
    ]
  },

  // --- RELATED FOCUS AREAS: ADR and Litigation ---

  "alternative-dispute-resolution": {
    mainImage: "/adr.png",
    title: "Alternative to Dispute Resolutions (ADR)",
    intro:
      "ADR refers to methods of settling disputes outside the traditional court system, including mediation, negotiation, arbitration, and conciliation. It focuses on communication, compromise, and efficiency to preserve relationships, save time, and reduce costs.",

    sections: [
      {
        heading: "What is ADR?",
        content: `
          Alternative Dispute Resolution (ADR) refers to methods of settling disputes outside the traditional court system. It includes processes such as mediation, negotiation, arbitration, and conciliation, where parties work collaboratively, often with the help of a neutral third party, to reach a fair, binding, or non-binding agreement.
          
          ADR focuses on communication, compromise, and efficiency, ensuring that conflicts are resolved in a manner that preserves relationships, saves time, and reduces the cost and stress associated with litigation.
        `
      },
      {
        heading: "How do we apply this to our clients?",
        content: `At our firm, ADR serves as a strategic tool to protect our clients’ interests while promoting peaceable and cost-effective solutions. We integrate ADR in several ways:`,
        subSections: [
          {
            title: "In Property Law",
            details:
              "Facilitating negotiations between landlords and tenants, or resolving boundary, lease, or ownership disputes without lengthy court battles."
          },
          {
            title: "In Family Law",
            details:
              "Using mediation and conciliation to handle sensitive matters such as divorce settlements, custody, and maintenance in a confidential and humane way."
          },
          {
            title: "In Entertainment Law",
            details:
              "Assisting artists, managers, and production companies to resolve contractual disagreements privately and maintain professional relationships."
          },
          {
            title: "In Corporate and Immigration Matters",
            details:
              "Managing compliance or contractual disputes through structured dialogue and arbitration."
          }
        ]
      },
      {
        heading: "Our ADR Philosophy",
        content: `Our approach prioritizes resolution over confrontation, ensuring that outcomes are fair, legally sound, and preserve the long-term interests of our clients.`
      }
    ]
  },

  litigation: {
    mainImage: "/litigation.png",
    title: "Litigation",
    intro:
      "Litigation refers to the process of resolving disputes through the court system. It is a formal legal mechanism where parties bring their cases before a judge or tribunal for determination. At our firm, litigation is always the last resort.",

    sections: [
      {
        heading: "What is Litigation?",
        content: `
          Litigation refers to the process of resolving disputes through the court system. It is a formal legal mechanism where parties bring their cases before a judge or tribunal for determination. Litigation may be civil (involving private disputes) or criminal (involving offences prosecuted by the state).
          
          At its core, litigation seeks justice and legal redress, but it can be time-consuming, emotionally draining, and financially demanding.
        `
      },
      {
        heading: "Our Policy on Litigation",
        content: `
          At our firm, litigation is always the last resort. We believe that not every conflict requires a courtroom. Our first priority is to help clients resolve disputes amicably through Alternative Dispute Resolution (ADR).
          
          We advise clients to exhaust all non-contentious options first. Only when resolution remains impossible do we turn to the courts. Where litigation becomes absolutely necessary, our firm maintains a network of trusted **external litigation partners** who handle the proceedings on our behalf, under our guidance and supervision.
        `
      },
      {
        heading: "Civil Actions",
        content: `Civil actions are legal proceedings between private individuals or organizations seeking to enforce a right, or to obtain compensation for a wrong (e.g., breach of contract, land ownership, debt recovery). The goal is to restore rights or recover losses, not to punish.`,
        subSections: [
          {
            title: "Pre-Action Stage",
            details:
              "Exploring settlement or ADR options; issuing a pre-action protocol or letter of demand."
          },
          {
            title: "Filing of Originating Processes",
            details:
              "Claimant files a Writ of Summons, Statement of Claim, and supporting documents at the appropriate court."
          },
          {
            title: "Pleadings Stage",
            details:
              "Parties exchange written pleadings (Statement of Claim, Defence, Reply), setting out their respective cases."
          },
          {
            title: "Trial and Judgment",
            details:
              "Parties present evidence and arguments, followed by the court delivering its binding decision."
          }
        ]
      },
      {
        heading: "Criminal Actions",
        content: `Criminal actions are legal proceedings initiated by the State against an individual or organization accused of committing an offence (e.g., theft, fraud, assault). The key objective is justice, deterrence, and the protection of the public.`,
        subSections: [
          {
            title: "Chronology",
            details:
              "Investigation → Arrest → Prosecution/Charging → Arraignment and Plea → Trial → Judgment → Sentencing."
          },
          {
            title: "Our Role in Criminal Matters",
            details:
              "We focus on **advisory, preventive, and compliance-related services**. We guide clients during investigations, assist with compliance policies (anti-corruption, data protection), and engage external criminal specialists if a matter proceeds to prosecution."
          }
        ]
      },
      {
        heading: "Advantages vs. Disadvantages",
        content: ``,
        subSections: [
          {
            title: "Advantages",
            details:
              "Provides a binding and enforceable judgment, allows for legal precedent, and offers structured procedural safeguards and appeal options."
          },
          {
            title: "Disadvantages",
            details:
              "Often expensive and time-consuming, can strain relationships, lacks flexibility (outcome strictly determined by the court), and involves public proceedings."
          }
        ]
      }
    ]
  }
};
